...
VectorVector StarMapValidation_SN::SetupIC() {
    ...
    VectorVector initialSolution = VectorVector( _mesh->GetNumCells(), Vector( _settings->GetNQuadPoints(), 0.0 ) );
    VectorVector cellMidpoints   = _mesh->GetCellMidPoints();
    const double stddev          = .01;
    double x = 0.0, y = 0.0, f = 0.0;

    for( unsigned idx_cell = 0; idx_cell < _mesh->GetNumCells(); ++idx_cell ) {
        x = cellMidpoints[idx_cell][0] - pos_beam[0];
        y = cellMidpoints[idx_cell][1] - pos_beam[1];
        f = 1.0 / ( 4.0 * M_PI * stddev ) * std::exp( -( x * x + y * y ) / ( 4 * stddev ) );
        // initialSolution[idx_cell][0] = f;    //* StarMAPmoments[0];
        for( unsigned idx_quad = 0; idx_quad < _settings->GetNQuadPoints(); idx_quad++ ) {
            initialSolution[idx_cell][idx_quad] = f;    //* StarMAPmoments[idx_sys];    // must be VectorVector
        }
    }
...
VectorVector StarMapValidation_Moment::SetupIC() {
    ...
    for( unsigned idx_cell = 0; idx_cell < _mesh->GetNumCells(); ++idx_cell ) {
        ...
        y = cellMidpoints[idx_cell][1] - pos_beam[1];
        f = 1.0 / ( 4.0 * M_PI * stddev ) * std::exp( -( x * x + y * y ) / ( 4 * stddev ) ) /
            ( 4.0 * M_PI );                  // NormPDF( x, pos_beam[0], stddev ) * NormPDF( y, pos_beam[1], stddev );
        initialSolution[idx_cell][0] = f;    //* StarMAPmoments[0];
