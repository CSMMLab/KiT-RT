...
ProblemBase* ProblemBase::Create(Config* settings, Mesh* mesh) {
  ...
  switch (settings->GetProblemName()) {
    ...
    case PROBLEM_Phantomimage:
      return new PhantomImage(settings, mesh);
    ...
    default:
      ErrorMessages::Error(
          "No valid physical problem chosen. Please check your config file",
          CURRENT_FUNCTION);
      return nullptr;
  }
}
...
// Default densities = 1
std::vector<double> ProblemBase::GetDensity(const VectorVector& cellMidPoints) {
  return std::vector<double>(cellMidPoints.size(), 1.0);
}

// Legacy code: Scattering crossection loaded from database ENDF with physics
// class -> later overwritten with ICRU data
VectorVector ProblemBase::GetScatteringXSE(const Vector& /*energies*/,
                                           const Vector& /*angles*/) {
...
void ProblemBase::SetGhostCells() {
  ...
  for (unsigned idx_cell = 0; idx_cell < _mesh->GetNumCells(); idx_cell++) {
    if (cellBoundaries[idx_cell] == BOUNDARY_TYPE::NEUMANN ||
        cellBoundaries[idx_cell] == BOUNDARY_TYPE::DIRICHLET) {
...
const Vector& ProblemBase::GetGhostCellValue(int idx_cell,
                                             const Vector& cell_sol) {
  return cell_sol;
}

// Needs to go to Solver, not to Problem
double ProblemBase::GetFinalTimeOutflow(const VectorVector& sol,
                                        const VectorVector& sol quadPoints,
                                        const VectorVector& sol quadPoints) {
  for (std::map<int, Vector>::iterator it = _ghostCells.begin();
       it != _ghostCells.end(); ++it) {  // Iterate over boundaries
    int idx_cell = it->first;            // Get Boundary cell index
    for (unsigned idx_nbr = 0; idx_nbr < _mesh->GetNeighbours[idx_cell].size();
         ++idx_nbr) {
      if (_mesh->GetNeighbours[idx_cell][idx_nbr] ==
          _nCells) {  // Find face that points outward
        for (unsigned idx_quad = 0; idx_quad < quadPoints.size(); ++idx_quad) {
          double fluxOut =
              _mesh->GetNormals()[idx_cell][idx_nbr] * quadPoints[idx_quad];
          if (fluxOut > 0.0) {
            fluxOut* sol[idx_cell][idx_quad]
          }
        }
      }
    }
...
double ProblemBase::GetFinalTimeAbsorption(const VectorVector& sol) {
  return 0;
}
...
double ProblemBase::GetTotalAbsorptionCenter(const VectorVector& sol) {
  return 0;
}

double ProblemBase::GetTotalAbsorptionVertical(const VectorVector& sol) {
  return 0;
}

double ProblemBase::GetTotalAbsorptionHorizontal(const VectorVector& sol) {
  return 0;
}
