...
VectorVector Waterphantom::SetupIC() {
    ...
    double s                      = 0.01;
    double s_x                      = 0.001;
    double enterPositionX         = 0.01;    // 0.0;
    ...
    for( unsigned j = 0; j < cellMids.size(); ++j ) {
        ...
        for( unsigned idx_quad = 0; idx_quad < _settings->GetNQuadPoints(); idx_quad++ ) {
           ...
           //if( x > -0.01 && x < 0.01 ) {
                ...
                psi[j][idx_quad] =  std::max(1.0 / ( s * s_x *s_ang * sqrt( 8 * M_PI * M_PI * M_PI ) ) * std::exp( -( x * x) / ( 2 * s_x * s_x ) )*std::exp( -(y * y ) / ( 2 * s * s ) ) *
                                                 std::exp( -( ang * ang ) / ( 2 * s_ang * s_ang) ),
                                             epsilon );
                // psi[j][idx_quad] =  std::max(1.0 / ( s * s_ang * sqrt( 4 * M_PI  * M_PI ) ) *std::exp( -(x * x + y * y ) / ( 2 * s * s ) ) *
                //                                  std::exp( -( ang * ang ) / ( 2 * s_ang * s_ang) ),
                //                              epsilon );
         // }
...
Waterphantom_Moment::Waterphantom_Moment( Config* settings, Mesh* mesh ) : Waterphantom( settings, mesh ) {}

Waterphantom_Moment::~Waterphantom_Moment() {}

std::vector<VectorVector> Waterphantom_Moment::GetExternalSource( const Vector& energies ) {
...
VectorVector Waterphantom_Moment::SetupIC() {
    if( _settings->GetSphericalBasisName() == SPHERICAL_HARMONICS ) {
        ...
        double enterPositionY         = 0.5;
        double meanDir                = 0;
        if(_settings->GetSolverName() == CSD_PN_SOLVER){
            meanDir                = meanDir + M_PI;
            }
        double s_ang                  = 0.01;
            ...
            for( unsigned idx_quad = 0; idx_quad < _settings->GetNQuadPoints(); idx_quad++ ) {
                ...
                //if( quadPointsSphere[idx_quad][0] < 0 ) {
                    ...
                    cellKineticDensity[idx_quad] =
                        std::max( 1.0 / ( s * s_x *s_ang * sqrt( 8 * M_PI * M_PI * M_PI ) ) * std::exp( -( x * x) / ( 2 * s_x * s_x ) )*std::exp( -(y * y ) / ( 2 * s * s ) ) *
                                                 std::exp( -( ang * ang ) / ( 2 * s_ang ) ),
                                             epsilon );
                    // cellKineticDensity[idx_quad] = std::max(1.0 / ( s * s_ang * sqrt( 4 * M_PI  * M_PI ) ) *std::exp( -(y * y) / ( 2 * s * s ) )*
                    //                              std::exp( -( ang * ang ) / ( 2 * s_ang * s_ang) ),
                    //                          epsilon );
              //}
                //}
