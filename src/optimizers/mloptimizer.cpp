...
MLOptimizer::MLOptimizer( Config* settings ) : OptimizerBase( settings ) {
    ...
    if( _settings->GetEnforceNeuralRotationalSymmetry() ) {
        if( _settings->GetMaxMomentDegree() != 2 ) {
            ErrorMessages::Error( "This postprocessing step is currently only for M2-2D models available.", CURRENT_FUNCTION );
        }
        servingSize *= 2;
...
Matrix MLOptimizer::CreateRotator( const Vector& uFirstMoment ) {
    double r = norm( uFirstMoment );
    double c = uFirstMoment[0] / r;
    double s = -uFirstMoment[1] / r;
    return Matrix{ { c, -s }, { s, c } };    // Rotation Matrix
}

void MLOptimizer::RotateM1( Vector& vec, Matrix& R ) { vec = R * vec; }

void MLOptimizer::RotateM2( Matrix& vec, Matrix& R ) { vec = R * vec * blaze::trans( R ); }
...
#pragma omp parallel for
        for( unsigned idx_cell = 0; idx_cell < _settings->GetNCells(); idx_cell++ ) {
            Vector u1{ _modelServingVectorU[idx_cell * ( _nSystem - 1 )], _modelServingVectorU[idx_cell * ( _nSystem - 1 ) + 1] };
            Matrix u2{ { _modelServingVectorU[idx_cell * ( _nSystem - 1 ) + 2], _modelServingVectorU[idx_cell * ( _nSystem - 1 ) + 3] },
                       { _modelServingVectorU[idx_cell * ( _nSystem - 1 ) + 3], _modelServingVectorU[idx_cell * ( _nSystem - 1 ) + 4] } };
            _rotationMats[idx_cell] = CreateRotator( u1 );
            RotateM1( u1, _rotationMats[idx_cell] );
            RotateM2( u2, _rotationMats[idx_cell] );
            // Save rotated moment
            _modelServingVectorU[idx_cell * ( _nSystem - 1 )]     = u1[0];
            _modelServingVectorU[idx_cell * ( _nSystem - 1 ) + 1] = u1[1];    // should be zero
            _modelServingVectorU[idx_cell * ( _nSystem - 1 ) + 2] = u2( 0, 0 );
            _modelServingVectorU[idx_cell * ( _nSystem - 1 ) + 3] = u2( 0, 1 );
            _modelServingVectorU[idx_cell * ( _nSystem - 1 ) + 4] = u2( 1, 1 );
            // Rotate Moment by 180 degrees and save mirrored moment
            _modelServingVectorU[( _settings->GetNCells() + idx_cell ) * ( _nSystem - 1 )]     = -u1[0];        // Only first moment is mirrored
            _modelServingVectorU[( _settings->GetNCells() + idx_cell ) * ( _nSystem - 1 ) + 1] = -u1[1];        // should be zero
            _modelServingVectorU[( _settings->GetNCells() + idx_cell ) * ( _nSystem - 1 ) + 2] = u2( 0, 0 );    // Even Moments cancel rotation
            _modelServingVectorU[( _settings->GetNCells() + idx_cell ) * ( _nSystem - 1 ) + 3] = u2( 0, 1 );
            _modelServingVectorU[( _settings->GetNCells() + idx_cell ) * ( _nSystem - 1 ) + 4] = u2( 1, 1 );
        }
        servingSize *= 2;
    }
...
#pragma omp parallel for
        for( unsigned idx_cell = 0; idx_cell < _settings->GetNCells(); idx_cell++ ) {
            ...
            for( unsigned idx_sys = 0; idx_sys < _nSystem - 1; idx_sys++ ) {
                ...
                if( idx_sys > 1 ) alphaRedMirror[idx_sys] *= -1;                            // Miror order 2 moments back
            ...
            Matrix alpha2{ { alphaRed[2], alphaRed[3] }, { alphaRed[3], alphaRed[4] } };
            Matrix rotBack = blaze::trans( _rotationMats[idx_cell] );
            RotateM1( alpha1, rotBack );
            RotateM2( alpha2, rotBack );
            // Store back-rotated alpha
            alphaRed[0] = alpha1[0];
            alphaRed[1] = alpha1[1];
            alphaRed[2] = alpha2( 0, 0 );
            alphaRed[3] = alpha2( 0, 1 );
