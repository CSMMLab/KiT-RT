...
SolverBase::SolverBase(Config *settings)
{
    ...
    _timeDependentOutflow = Vector(_maxIter, 0.0);
...
SolverBase *SolverBase::Create(Config *settings)
{
    ...
    case MN_SOLVER:
        return new MNSolver(settings);
    case MN_SOLVER_NORMALIZED:
        return new MNSolverNormalized(settings);
    case CSD_SN_SOLVER:
        return new CSDSNSolver(settings);
    case CSD_PN_SOLVER:
        return new CSDPNSolver(settings);
    case CSD_MN_SOLVER:
        return new CSDMNSolver(settings);
...
void SolverBase::Solve()
{
    ...
    for (unsigned iter = 0; iter < _maxIter; iter++)
    {
        ...
        if (rkStages == 2)
            RKUpdate(solRK0, _sol);

        // --- Solver Output ---
        WriteVolumeOutput(iter);
        WriteScalarOutput(iter);
        // --- Update Fluxes (not sure if needed)
        _flux = _fluxNew;
...
void SolverBase::PrintVolumeOutput(int currEnergy) const
{
    ...
    if (currEnergy == (int)_maxIter - 1)
    { // Last iteration write without suffix.
...
void SolverBase::PrepareScreenOutput()
{
    ...
    for (unsigned idx_field = 0; idx_field < nFields; idx_field++)
    {
        ...
        {
        case MASS:
            _screenOutputFieldNames[idx_field] = "Mass";
            break;
        case ITER:
            _screenOutputFieldNames[idx_field] = "Iter";
            break;
        case RMS_FLUX:
            _screenOutputFieldNames[idx_field] = "RMS flux";
            break;
        case VTK_OUTPUT:
            _screenOutputFieldNames[idx_field] = "VTK out";
            break;
        case CSV_OUTPUT:
            _screenOutputFieldNames[idx_field] = "CSV out";
            break;
        case FINAL_TIME_OUTFLOW:
            _screenOutputFieldNames[idx_field] = "Final time outflow";
            break;
        case TOTAL_OUTFLOW:
            _screenOutputFieldNames[idx_field] = "Cumulated outflow";
            break;
        case MAX_OUTFLOW:
            _screenOutputFieldNames[idx_field] = "Max outflow";
            break;
        case FINAL_TIME_PARTICLE_ABSORPTION:
            _screenOutputFieldNames[idx_field] = "Final time absorption";
            break;
        case TOTAL_PARTICLE_ABSORPTION:
            _screenOutputFieldNames[idx_field] = "Cumulated absorption";
            break;
        case MAX_PARTICLE_ABSORPTION:
            _screenOutputFieldNames[idx_field] = "Max absorption";
            break;
        case TOTAL_PARTICLE_ABSORPTION_CENTER:
            _screenOutputFieldNames[idx_field] = "Cumulated absorption center";
            break;
        case TOTAL_PARTICLE_ABSORPTION_VERTICAL:
            _screenOutputFieldNames[idx_field] = "Cumulated absorption vertical wall";
            break;
        case TOTAL_PARTICLE_ABSORPTION_HORIZONTAL:
            _screenOutputFieldNames[idx_field] = "Cumulated absorption horizontal wall";
            break;

        default:
            ErrorMessages::Error("Screen output field not defined!", CURRENT_FUNCTION);
            break;
        }
    }
}
...
void SolverBase::WriteScalarOutput(unsigned iteration)
{
    ...
    for (unsigned idx_field = 0; idx_field < nFields; idx_field++)
    {
        ...
        case ITER:
            _screenOutputFields[idx_field] = iteration;
            break;
        ...
        case CSV_OUTPUT:
            ...
            break;
        case FINAL_TIME_OUTFLOW:
            _screenOutputFields[idx_field] = GetCurrentOutflow();
            break;
        case TOTAL_OUTFLOW:
            _screenOutputFields[idx_field] = GetTotalOutflow(iteration);
            break;
        case MAX_OUTFLOW:
            _screenOutputFields[idx_field] = GetMaxOutflow();
            break;
        case FINAL_TIME_PARTICLE_ABSORPTION:
            _screenOutputFields[idx_field] = GetFinalTimeAbsorption();
            break;
        case TOTAL_PARTICLE_ABSORPTION:
            _screenOutputFields[idx_field] = GetTotalAbsorption();
            break;
        case MAX_PARTICLE_ABSORPTION:
            _screenOutputFields[idx_field] = GetMaxAbsorption();
            break;
        case TOTAL_PARTICLE_ABSORPTION_CENTER:
            _screenOutputFields[idx_field] = GetTotalAbsorptionCenter();
            break;
        case TOTAL_PARTICLE_ABSORPTION_VERTICAL:
            _screenOutputFields[idx_field] = GetTotalAbsorptionVertical();
            break;
        case TOTAL_PARTICLE_ABSORPTION_HORIZONTAL:
            _screenOutputFields[idx_field] = GetTotalAbsorptionHorizontal();
            break;
        default:
            ErrorMessages::Error("Screen output group not defined!", CURRENT_FUNCTION);
            break;
        }
    }
    ...
    for (unsigned idx_field = 0; idx_field < nFields; idx_field++)
    {
        ...
        case ITER:
            _historyOutputFields[idx_field] = iteration;
            break;
        ...
        case CSV_OUTPUT:
            ...
            break;
        case FINAL_TIME_OUTFLOW:
            _historyOutputFields[idx_field] = GetCurrentOutflow();
            break;
        case TOTAL_OUTFLOW:
            _historyOutputFields[idx_field] = GetTotalOutflow(iteration);
            break;
        case MAX_OUTFLOW:
            _historyOutputFields[idx_field] = GetMaxOutflow();
            break;
        case FINAL_TIME_PARTICLE_ABSORPTION:
            _historyOutputFields[idx_field] = GetFinalTimeAbsorption();
            break;
        case TOTAL_PARTICLE_ABSORPTION:
            _historyOutputFields[idx_field] = GetTotalAbsorption();
            break;
        case MAX_PARTICLE_ABSORPTION:
            _historyOutputFields[idx_field] = GetMaxAbsorption();
            break;
        case TOTAL_PARTICLE_ABSORPTION_CENTER:
            _historyOutputFields[idx_field] = GetTotalAbsorptionCenter();
            break;
        case TOTAL_PARTICLE_ABSORPTION_VERTICAL:
            _historyOutputFields[idx_field] = GetTotalAbsorptionVertical();
            break;
        case TOTAL_PARTICLE_ABSORPTION_HORIZONTAL:
            _historyOutputFields[idx_field] = GetTotalAbsorptionHorizontal();
            break;
        default:
            ErrorMessages::Error("History output group not defined!", CURRENT_FUNCTION);
            break;
        }
    }
}
...
void SolverBase::PrintScreenOutput(unsigned iteration)
{
    ...
    for (unsigned idx_field = 0; idx_field < _settings->GetNScreenOutput(); idx_field++)
    {
        ...
        {
            tmp = "no";
            if ((bool)_screenOutputFields[idx_field])
                tmp = "yes";
        }
...
void SolverBase::PrepareHistoryOutput()
{
    ...
    for (unsigned idx_field = 0; idx_field < nFields; idx_field++)
    {
        ...
        {
        case MASS:
            _historyOutputFieldNames[idx_field] = "Mass";
            break;
        case ITER:
            _historyOutputFieldNames[idx_field] = "Iter";
            break;
        case RMS_FLUX:
            _historyOutputFieldNames[idx_field] = "RMS_flux";
            break;
        case VTK_OUTPUT:
            _historyOutputFieldNames[idx_field] = "VTK_out";
            break;
        case CSV_OUTPUT:
            _historyOutputFieldNames[idx_field] = "CSV_out";
            break;
        case FINAL_TIME_OUTFLOW:
            _historyOutputFieldNames[idx_field] = "Final_time_outflow";
            break;
        case TOTAL_OUTFLOW:
            _historyOutputFieldNames[idx_field] = "Cumulated_outflow";
            break;
        case MAX_OUTFLOW:
            _historyOutputFieldNames[idx_field] = "Max_outflow";
            break;
        case FINAL_TIME_PARTICLE_ABSORPTION:
            _historyOutputFieldNames[idx_field] = "Final_time_absorption";
            break;
        case TOTAL_PARTICLE_ABSORPTION:
            _historyOutputFieldNames[idx_field] = "Cumulated_absorption";
            break;
        case MAX_PARTICLE_ABSORPTION:
            _historyOutputFieldNames[idx_field] = "Max_absorption";
            break;
        case TOTAL_PARTICLE_ABSORPTION_CENTER:
            _historyOutputFieldNames[idx_field] = "Cumulated_absorption_center";
            break;
        case TOTAL_PARTICLE_ABSORPTION_VERTICAL:
            _historyOutputFieldNames[idx_field] = "Cumulated_absorption_vertical_wall";
            break;
        case TOTAL_PARTICLE_ABSORPTION_HORIZONTAL:
            _historyOutputFieldNames[idx_field] = "Cumulated_absorption_horizontal_wall";
            break;
        default:
            ErrorMessages::Error("History output field not defined!", CURRENT_FUNCTION);
            break;
        }
    }
}
...
void SolverBase::PrintHistoryOutput(unsigned iteration)
{
    ...
    if (rank == 0)
    {
        ...
        else if (iteration == _nEnergies - 1)
        { // Always print last iteration
            log->info(lineToPrint);
        }
    }
