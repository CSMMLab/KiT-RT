...
BOUNDARY_TYPE Config::GetBoundaryType(std::string name) const
{
    ...
    {
        if (name == _boundaries[i].first)
            return _boundaries[i].second;
    }
...
void Config::SetPostprocessing()
{
    ...
    if (_logDir[_logDir.size() - 1] != '/')
        _logDir.append("/");
    if (_outputDir[_outputDir.size() - 1] != '/')
        _outputDir.append("/");
    if (_inputDir[_inputDir.size() - 1] != '/')
        _inputDir.append("/");
    ...
    if (!std::filesystem::exists(_outputDir))
        std::filesystem::create_directory(_outputDir);
    ...
    case PN_SOLVER:
    case CSD_PN_SOLVER:
        _isMomentSolver = true;
        break;
    default:
        _isMomentSolver = false;
    }
    ...
    // Optimizer postprocessing
    {
        ...
        if (_entropyOptimizerName == ML)
        {
            ...
            case 0:
                ...
                break;
            case 1:
                _regularizerGamma = 0.1;
                break;
            case 2:
                _regularizerGamma = 0.01;
                break;
            case 3:
                _regularizerGamma = 0.001;
                break;
            }
        }
