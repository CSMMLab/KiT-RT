...
def main():
    ...
    parser.add_option(
        ...
        "--output_name",
        dest="output_name",
        default="struct_mesh_symmetric_hohlraum",
    )
    parser.add_option("-c", "--char_length", dest="char_length", default=0.01)
    parser.add_option(
        "-s", "--start_pt", dest="start_pt", nargs=2, default=(-0.65, -0.65)
    )
    ...
    if options.b_type == "void":
        ...
        geom.add_physical(dirichlet_wall_up, label="wall_low")
        geom.add_physical(dirichlet_wall_low, label="wall_up")
        geom.add_physical(neumann_wall_left, label="wall_left")
        geom.add_physical(neumann_wall_right, label="wall_right")
        print("| Wall marker have tags wall_left, wall_right, wall_up, and wall_low")
