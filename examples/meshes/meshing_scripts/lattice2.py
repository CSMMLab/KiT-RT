...
def main(filename, lc, sigma_t):
    # Specify domain
    refinement_box_width = lc
    transition_thickness = 5 * lc
    ...
    for (b_x_i, b_y_i) in zip(box_x, box_y):  # Generate mesh refinement boxes around the absortion boundaries
        ...
        fields.append(field)
    min_field = gmsh.model.mesh.field.add("Min")  # use the minimum of all the fields as the background mesh field:
    ...
    # gmsh.model.mesh.setRecombine(2, surface_marker)
...
if __name__ == '__main__':
    ...
    parser.add_option("-c", "--char_length", dest="char_length", default=0.1)
