...
//level makes reference to the level of refinement (mesh or quadrature), NnMCSamples number of iterations in the montecarlo, 
void ML_l(int level, int nMCSamples, double *sums, Config* config ) {
    ...
    if(level==0){
        ...
        for( unsigned k = 0; k < nMCSamples; ++k ) {
          ...
          solverc->SetNewQN(pow(2, 1+level));             
...
int main( int argc, char** argv ) {
    ...
    if( config->GetDataGeneratorMode() ) {
      ...
      */    
      ...
      FILE *fp;
      fp = fopen("KIT-RT mlmc","w");
      
      
      float Eps[11];
      float Eps2[] = { 10, 1, 0.5, 0.0 };        

      memcpy(Eps,Eps2,sizeof(Eps2));        

      int M  = 3;     // refinement cost factor
      int N0 = 10;    // initial samples on each level
      int Lmin = 2;   // minimum refinement level
      int Lmax = 6;   // maximum refinement level
      int N  = 3;    // samples for convergence tests
      int L  = 5;     // levels for convergence tests
      int NL = 10;
      float Cl = 2.0;
      float eps = 1.0;                

      //double P = mlmc(Lmin,Lmax,N0,eps,ML_l,config,0.0,0.0,0.0,&NL,&Cl);

      mlmc_test(ML_l,config,M,N,L,N0,Eps,Lmin,Lmax,fp); 
...
/*        // Build solver
        ...
        for( unsigned k = 0; k < nMCSamples; ++k ) {
            ...
            solver->SetNewQN(20);
            std::cout<< solver->GetNQ()<<std::endl;
        ...
        //print_result(expDosis);
        double maxDose = *std::max_element( expDosis.begin(), expDosis.end() );
        //print_result(expDosis);
        std::cout<< maxDose<<std::endl;
