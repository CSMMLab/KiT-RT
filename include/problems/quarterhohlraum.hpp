#ifndef QUARTER_HOHLRAUM_H
#define QUARTER_HOHLRAUM_H
...
class QuarterHohlraum : public ProblemBase
{
  ...
  protected:
    ...
    double _redRightTop;                        /*!< @brief x coord of the right top corner of the red area */
  ...
  public:
    QuarterHohlraum( Config* settings, Mesh* mesh, QuadratureBase* quad );
    virtual ~QuarterHohlraum();
...
{
  private:
    QuarterHohlraum_Moment() = delete;
    // TODO void SetGhostCells() override final; /*!< @brief Sets vector of ghost cells for boundary conditions */

  public:
    QuarterHohlraum_Moment( Config* settings, Mesh* mesh, QuadratureBase* quad );
    ~QuarterHohlraum_Moment();
